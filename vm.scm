...
(define (function-asm function ir lifetimes)
  ...
  (define asm '())
  (define dead-code #f)
  (define (die!) (when (not inline-context) (set! dead-code #t)))
      ...
      (let* ([call-cost (call/cc (λ (cc) (λ (x) (restore) (cc x))))]
        ...
        (cond
         ...
         [else
          ...
          (let ([out (reference! fn line out-var)])
...
(run-l5
 ...
 (λ (asm registers ram)
   (> .8 (memory-usage ram))
   (= 180 (read-register registers 'ret))))
